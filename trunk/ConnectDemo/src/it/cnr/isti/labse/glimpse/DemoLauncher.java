...
public class DemoLauncher {
	...
	public static boolean initProps()
	{
		...
		{			
			...
			ENABLER1PARAMETERSFILE = systemProps.getProperty("ENABLER1PARAMETERSFILE");
			ENABLER2PARAMETERSFILE = systemProps.getProperty("ENABLER2PARAMETERSFILE");
	...
	public static void main(String[] args) {
		...
		if (DemoLauncher.initProps() && DemoLauncher.init())
		{
			...
			ComplexEventProcessor engine = new DroolsComplexEventProcessor(Manager.Read(MANAGERPARAMETERFILE), buffer, connFact, initConn);
			engine.start();
			...
			ConnectEnablersManager manager = new ConnectEnablersManager(Manager.Read(MANAGERPARAMETERFILE), connFact, initConn, engine.getRuleManager());
			manager.start();
			...
			SimpleConnectEnabler testingConsumer1 = new SimpleConnectEnabler(Manager.Read(ENABLER1PARAMETERSFILE), connFact, initConn);
			...
			SimpleConnectEnabler testingConsumer2 = new SimpleConnectEnabler(Manager.Read(ENABLER2PARAMETERSFILE), connFact, initConn);
